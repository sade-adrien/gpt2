...
def clean_control_char(string):
    ...
    for char in string:
        if unicodedata.category(char)[0] == 'C':
            chars.append(f"\\u{ord(ch):04x}")           # unknown utf-8 char
        else:
            chars.append(char)
    return chars
...
class GPT2Tokenizer(Tokenizer):
    ...
    def train(self, text, vocab_size, verbose=False):
        ...
        for i in range(num_merges):
            ...
            if verbose:
                print(f"Merge {i+1}/{num_merges}: `{vocab[max_pair[0]]}` (token [{max_pair[0]}]) + `{vocab[max_pair[1]]}` (token [{max_pair[1]}]) --> `{vocab[new_id]}` (token [{new_id}])")
